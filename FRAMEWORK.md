...
This document is not a thesis or motivational argument. For the design rationale, system properties, and analysis, see the accompanying thesis document [*Privacy Preserving Compliance*](https://github.com/JossDuff/thesis).
...
A reference implementation accompanies this specification at [https://github.com/JossDuff/Privacy-Preserving-Compliance](https://github.com/JossDuff/Privacy-Preserving-Compliance). It makes the following implementation choices:
...
Blockchain privacy and regulatory compliance present a fundamental tension: privacy requires that only the individual knows certain information, while compliance traditionally requires revealing information. This framework resolves this conflict by enabling users to prove compliance cryptographically using zero-knowledge proofs.
...
- The `verify` function MAY revert on an invalid proof.
...
| `tStart` | Block height at which this version becomes active. MAY be set in the future to publish a version before it takes effect. MUST be greater than `tEND`. |
| `tEnd` | Block height at which this version expires. Set to `type(uint256).max` for no expiration.  MUST be less than `tStart`. |
...
Verifies a user's proof against the currently active version. This is the function that applications call to gate access.  TODO: optionally add extra params.
...
5. **Succinctness**: The on-chain verification cost MUST NOT grow with the complexity of the compliance definition.

### 6.2 Trusted Setup Considerations

Proof systems that require a per-circuit trusted setup (e.g., Groth16) are permitted but impractical for this framework.  Each compliance definition update that change the underlying circuit would require a new trusted setup ceremony.
...
2. **Public availability**: Artifacts MUST be publicly retrievable by any user who needs to generate a proof. 
...
The `circuitRef` field in `ComplianceVersion` points to the circuit artifacts needed for proof generation. The artifact MUST contain sufficient information for a user to:

1. Determine the circuit's public and private input schema.
2. Generate a witness.
3. Generate a proof.

The specific format of circuit artifacts depends on the proving system. For Noir/Barretenberg, it could be a JSON file containing `bytecode` and `abi` fields or the full Noir code for users to compile themselves. Other proving systems will have their own artifact formats.
...
1. **Define constraints**: Express the compliance logic as one or more constraints (§4.1). Document the constraints in their formal notation.  The regulator might first want to express constraints in a human-readable format and compile that into constraint logic.
2. **Construct circuit**: Implement the compliance logic as a ZK circuit in the chosen circuit language. The circuit MUST define which inputs are public and which are private.  The regulator might use a compiler to generate a circuit given a defined set of constraints.
...
7. **Upload artifacts to content-addressable storage**:
   - Upload the circuit artifacts → obtain `circuitRef`.
...
1. Write, compile, and generate a new verification key for the updated circuit. (Steps 1-4 of §8.1)
...
2. **Store references**: Store the `ComplianceDefinition` contract address(es) in the application contract.
...
### 8.6 Lifecycle Sequence (TODO: this is all messed up)
...
For a full reference implementation of the framework, see [https://github.com/JossDuff/Privacy-Preserving-Compliance](https://github.com/JossDuff/Privacy-Preserving-Compliance).
